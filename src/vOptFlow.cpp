...
void vFlowManager::onRead(eventdriven::vBottle &inBottle)
{
    ...
    for(eventdriven::vQueue::iterator qi = q.begin(); qi != q.end(); qi++)
    {
        ...
        eventdriven::FlowEvent *ofe = compute();
//        eventsComputed++;
//        eventsPotential++;
...
eventdriven::FlowEvent *vFlowManager::compute()
{
    eventdriven::FlowEvent * vf = NULL;
    double dtdy = 0, dtdx = 0;

    //get the most recent event
    eventdriven::AddressEvent * vr =
            cSurf->getMostRecent()->getAs<eventdriven::AddressEvent>();
    ...
    for(int i = vr->getX()-fRad; i <= vr->getX()+fRad; i+=fRad) {
        for(int j = vr->getY()-fRad; j <= vr->getY()+fRad; j+=fRad) {
            ...
            const eventdriven::vQueue &subsurf = cSurf->getSurf(i, j, fRad);
    ...
    if(bestscore > eventdriven::vtsHelper::maxStamp()) return vf;

    const eventdriven::vQueue &subsurf = cSurf->getSurf(besti, bestj, fRad);

    //and compute the flow
    if(computeGrads(subsurf, *vr, dtdy, dtdx) >= minEvtsOnPlane) {
        vf = new eventdriven::FlowEvent(*vr);
        vf->setVx(dtdx);
        vf->setVy(dtdy);
        vf->setDeath();
    }
...
//            eventdriven::FlowEvent *vf2 = (*qi)->getAs<eventdriven::FlowEvent>();
//            if(!vf2) continue;
